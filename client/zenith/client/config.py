...
class InitConfig(
    ...
    #: Either the identity already exists or a new keypair is generated at the specified
    #: location
...
class ConnectConfig(
    ...
    run_as_user: conint(gt=0) | None = None
    #: The path to an SSH identity file to use
    ssh_identity_path: FilePath | None = None
    #: The SSH private key to use
    ssh_private_key_data: str | None = Field(None, validate_default=True)
    ...
    auth_oidc_issuer: AnyHttpUrl | None = None
    #: The OIDC client ID, if known
    auth_oidc_client_id: constr(min_length=1) | None = None
    #: The OIDC client secret, required if client ID is given
    auth_oidc_client_secret: constr(min_length=1) | None = None
    ...
    auth_oidc_allowed_groups: list[constr(pattern=r"^[a-zA-Z0-9_/-]+$")] = Field(
        default_factory=list
    )
    #: Parameters for the proxy authentication service
    auth_external_params: dict[AuthParamsKey, AuthParamsValue] = Field(
        default_factory=dict
    )
    #: Path to a file containing a TLS certificate chain to use
    tls_cert_file: FilePath | None = None
    #: Base64-encoded TLS certificate to use
    tls_cert_data: str | None = Field(None, validate_default=True)
    #: Path to a file containing a TLS certificate key to use
    tls_key_file: FilePath | None = None
    #: Base64-encoded TLS certificate key to use
    tls_key_data: str | None = Field(None, validate_default=True)
    #: Path to a file containing a CA to use to validate TLS client certificates
    tls_client_ca_file: FilePath | None = None
    #: Base64-encoded CA to use to validate TLS client certificates
    tls_client_ca_data: str | None = Field(None, validate_default=True)
