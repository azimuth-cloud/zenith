...
#: This will become a header name, so limit to lowercase alphanumeric + -
...
class ClientConfig(BaseModel, extra="forbid"):
    ...
    auth_oidc_client_id: constr(min_length=1) | None = Field(
        None, validate_default=True
    )
    #: The OIDC client secret to use
    auth_oidc_client_secret: constr(min_length=1) | None = Field(
        None, validate_default=True
    )
    ...
    auth_oidc_allowed_groups: list[AllowedGroup] = Field(default_factory=list)
    #: Parameters for the external authentication service (deprecated name)
    auth_params: dict[AuthParamsKey, AuthParamsValue] = Field(default_factory=dict)
    ...
    tls_key: str | None = Field(None, validate_default=True)
    ...
    def as_sync_config(self) -> dict[str, typing.Any]:
        """
