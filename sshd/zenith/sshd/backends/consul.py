...
class Backend(base.Backend):
    ...
    def __init__(
        self, logger: logging.Logger, url: str, key_prefix: str, service_tag: str
    ):
        self.logger = logger
        self.url = url
    ...
    def tunnel_init(
        ...
        self.logger.debug(
            "Posting configuration to Consul", extra={"tunnelid": tunnel_id}
        )
        url = "{consul_url}/v1/kv/{key_prefix}/{tunnel_id}?acquire={tunnel_id}".format(  # noqa: UP032
            consul_url=self.url, key_prefix=self.key_prefix, tunnel_id=tunnel_id
        )
        ...
        self.logger.debug(
            "Registering service with Consul", extra={"tunnelid": tunnel_id}
        )
        response = requests.put(
            ...
            json={
                ...
                "Check": {
                    ...
                    "TTL": f"{ttl}s",
                    # This deregisters the service once it has been critical for the
                    # specified interval
