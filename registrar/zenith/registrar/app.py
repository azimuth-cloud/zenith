...
def generate_random_subdomain():
    ...
    The subdomain must be a valid Kubernetes service name, so must be at most 63
    characters long, consist of lowercase letters, numbers and hyphens (-) only,
    and start with a letter.

    In addition, the FQDN (i.e. subdomain + base domain) must be at most 64 characters
    to fit in the CN of an SSL certificate when ACME issuing is used. Given that the
    base domain must be at least one character, plus a dot to separate the subdomain,
    this means that when a subdomain passes this test it automatically passes the length
    test for a Kubernetes service.
    """
    subdomain_len = min(
...
async def reserve_subdomain(request: Request, req: ReservationRequest | None = None):
    """
    ...
    while remaining_attempts > 0:
        ...
        subdomain = (
            req.subdomain if req.subdomain is not None else generate_random_subdomain()
        )
        ...
        raise HTTPException(
            status_code=409,
            detail="Unable to allocate a subdomain after {} attempts.".format(  # noqa: UP032
    ...
    if req.public_key:
        try:
            ...
            raise HTTPException(
                status_code=409, detail="Unable to associate public key."
            )
    ...
    if req.public_key:
        ...
        return Reservation(
            subdomain=subdomain, fqdn=fqdn, internal_fqdn=internal_fqdn, token=token
        )
...
async def verify_subdomain(req: VerificationRequest):
    ...
    """
    try:
        subdomain = await backend.subdomain_for_public_key(
            fingerprint_bytes(req.public_key)
        )
    except backends.PublicKeyNotAssociated:
        raise HTTPException(
            status_code=404, detail="The given SSH public key is not known."
        )
...
async def associate_public_key(req: PublicKeyAssociationRequest):
    ...
    #   2. Old-style tokens consisting of a subdomain, a modify index and a SHA1
    #      signature
    #
    # We only want to accept new-style tokens, as continuing to accept old-style tokens
    # would mean we are reducing our security to SHA1 (i.e. if you want to forge a
    # token, just use an old-style token).
    #
