...
class RegistrarConfig(
    ...
    #: The FQDN (i.e. subdomain + base domain) for each service must be at most 64
    #: characters to fit in the CN of an SSL certificate when ACME issuing is used
    #: Because we want to have at least 16 characters of randomness to use for
    #: subdomains, we limit the base domain to 47 characters (16 for subdomain plus a
    #: joining dot)
    ...
    reserved_subdomains: list[str] = Field(default_factory=list)
    ...
    #: have valid names (i.e. at most 53 characters), as that has a customisable
    #: template
