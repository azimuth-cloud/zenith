...
from .config import SSHPublicKeyType, settings
...
#: Subdomains must be at most 63 characters long, can only contain alphanumeric
#: characters and hyphens, and cannot start or end with a hyphen
#: In addition, this will eventually become a Kubernetes service name and Kubernetes
#: service names must start with a letter and be lowercase
...
def validate_ssh_key(v):
    ...
    if (
        key_type == SSHPublicKeyType.RSA
        and public_key.key_size < settings.ssh_rsa_min_bits
    ):
        message = "RSA keys must have a minimum of {} bits ({} given).".format(  # noqa: UP032
            settings.ssh_rsa_min_bits, public_key.key_size
        )
...
class ReservationRequest(BaseModel):
    ...
    def validate_legacy_public_keys(cls, data: t.Any):
        ...
        if (
            ...
            and "public_key" not in data
            and "public_keys" in data
            and isinstance(data["public_keys"], list)
            and len(data["public_keys"]) == 1
        ):
...
class Reservation(BaseModel):
    ...
    #: The token to use to associate public keys with the subdomain if no keys were
    #: given
    token: str | None = None
...
class VerificationResult(BaseModel):
    """
    Model for a verification result indicating the associated subdomain for a public
    key.
    """
