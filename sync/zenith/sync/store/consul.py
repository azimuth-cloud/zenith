...
class Store(base.Store):
    ...
    async def _config(self, instance):
        ...
        if 200 <= response.status_code < 300:
            ...
            self.logger.error("Error fetching config for service instance %s", service_id)
    ...
    async def _produce_events(self, list_idx, initial_services):
        ...
        service_tasks = {
            service.name: asyncio.create_task(self._wait_service(service.name, service_idx))
        ...
        while True:
            ...
            for completed_task in done:
                if completed_task == services_task:
                    ...
                    self.logger.info("Service query task completed for %s", service.name)
